...
void placeShip(char grid[GRID_SIZE][GRID_SIZE], int shipSize, int player, int isAI) {
    int x, y, orientation;
...
int main() {
    ...
    if (gameMode == 1) {
        for (int i = 1; i <= SHIP_COUNT; i++) {
            placeShip(player2Grid, i, 2, 0);
            displayFullGrid(player2Grid);
        }
    } else {
        for (int i = 1; i <= SHIP_COUNT; i++) {
            placeShip(player2Grid, i, 2, 1);  // AI places ships
        }
    }
